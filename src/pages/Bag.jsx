...
const Bag = () => {
  ...
  // Filter only bag products
  const bagProducts = products.filter(
    (p) => p.category?.toLowerCase() === "bag" // ya aapke product data me brand/type "Bag" ho
  );

  // Pagination logic
  const totalPages = Math.ceil(bagProducts.length / show);
  const indexOfLastProduct = currentPage * show;
  const indexOfFirstProduct = indexOfLastProduct - show;
  const currentProducts = bagProducts.slice(
    indexOfFirstProduct,
    indexOfLastProduct
  );
  ...
  return (
    <div className="min-h-screen p-4 mt-[65px] flex flex-col gap-6">
      <h1 className="text-2xl font-bold mb-4">Bag Page</h1>
      <p className="mb-4">Check out our collection of bags.</p>
